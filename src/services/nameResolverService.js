...
class NameResolverService {
  ...
  async resolveIdByName(
    ...
    try {
      ...
      if (folderName) {
        const folderResult = await this.resolveFolderId(
          accessToken,
          resolution.driveId,
          folderName
        );
      ...
      if (fileName) {
        const fileResult = await this.resolveFileId(
          accessToken,
          resolution.driveId,
          resolution.folderId,
          fileName,
      ...
      if (sheetName && resolution.itemId) {
        resolution.sheetId = await this.resolveSheetId(
          accessToken,
          resolution.driveId,
          resolution.itemId,
          sheetName
  ...
  async resolveDriveId(accessToken, driveName) {
    ...
    try {
      ...
      const match = (drives || []).find(
        (d) => String(d.name).toLowerCase() === driveNameLc
      );

      if (!match) {
        const available = (drives || []).map((d) => d.name);
        throw new AppError(
          `Drive '${driveName}' not found. Available drives: ${available.join(
            ", "
          )}`,
          404
        );
      ...
      throw new AppError(
        `Failed to resolve drive '${driveName}': ${err.message}`,
        err.status || 500
      );
    }
  }
  ...
  async resolveFolderId(accessToken, driveId, folderName) {
    ...
    try {
      ...
      const matches = await this.recursiveSearchForFolder(
        graphClient,
        driveId,
        folderName
      );
      ...
      const pathOptions = matches
        .map((match, index) => `${index + 1}. ${match.path}`)
        .join("\n");
      ...
      throw new AppError(
        `Failed to resolve folder '${folderName}': ${err.message}`,
        err.status || 500
      );
    }
  }
    ...
    try {
      ...
      if (folderId && folderId !== "root") {
        ...
        const directMatch = (folderItems.value || []).find(
          (item) =>
            !item.folder && String(item.name).toLowerCase() === fileNameLc
        );

        if (directMatch) {
          const result = {
            id: directMatch.id,
            path: `${folderPath}${folderPath.endsWith("/") ? "" : "/"}${
              directMatch.name
            }`,
          };
      ...
      const matches = await this.recursiveSearchForFile(
        graphClient,
        driveId,
        fileName
      );
      ...
      throw new AppError(
        `Failed to resolve file '${fileName}': ${err.message}`,
        err.status || 500
      );
    }
  }
  ...
  async resolveSheetId(accessToken, driveId, itemId, sheetName) {
    ...
    try {
      ...
      throw new AppError(
        `Failed to resolve sheet '${sheetName}': ${err.message}`,
        err.status || 500
      );
    }
  }

  async recursiveSearchForFolder(
    graphClient,
    driveId,
    folderName,
    currentPath = "",
    ...
    if (depth > maxDepth) {
      logger.warn(`Maximum search depth reached for folder '${folderName}'`, {
        driveId,
        currentPath,
      });
      return [];
  ...
  async recursiveSearchForFile(
    ...
    if (depth > maxDepth) {
      logger.warn(`Maximum search depth reached for file '${fileName}'`, {
        driveId,
        currentPath,
      });
      return [];
  ...
  async resolveByPath(accessToken, driveName, fullPath) {
    ...
    for (let i = 0; i < pathParts.length - 1; i++) {
      ...
      const folderResult = await this.findItemInFolder(
        accessToken,
        driveId,
        currentFolderId,
        folderName,
      ...
      if (!folderResult) {
        throw new AppError(
          `Folder '${folderName}' not found in path '${currentPath}'`,
          404
        );
      }
    ...
    const finalItem = await this.findItemInFolder(
      accessToken,
      driveId,
      currentFolderId,
      finalItemName,
    ...
    if (!finalItem) {
      throw new AppError(
        `Item '${finalItemName}' not found in path '${fullPath}'`,
        404
      );
    }
  ...
  async findItemInFolder(
    ...
    try {
      ...
      logger.error("Failed to find item in folder", {
        driveId,
        folderId,
        itemName,
        error: err.message,
