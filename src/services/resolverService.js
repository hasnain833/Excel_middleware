...
class ResolverService {
  ...
  async resolveDriveIdByName(accessToken, driveName) {
    ...
    try {
      ...
      if (!match) {
        const msg = `Drive not found. Available drives: ${JSON.stringify(
          available
        )}`;
        logger.warn(msg);
      ...
      if (!err.status)
        logger.error("Failed resolving driveId by name", {
          driveName,
          error: err.message,
        });
      throw err;
  ...
  async resolveItemIdByName(accessToken, driveId, itemName) {
    ...
    try {
      ...
      const rootMatch = rootItems.find(
        (it) => String(it.name).toLowerCase() === itemNameLc && !it.folder
      );
      ...
      logger.info(
        `File '${itemName}' not found in root folder. Starting recursive search...`,
        { driveId }
      );
      ...
      if (matches.length === 0) {
        const rootAvailable = rootItems
          .filter((it) => !it.folder)
          .map((it) => it.name);
        const msg = `File '${itemName}' not found in drive. Files in root folder: ${JSON.stringify(
          rootAvailable
        )}`;
      ...
      if (matches.length === 1) {
        ...
        logger.info(`File '${itemName}' found at path: ${match.path}`, {
          driveId,
          itemId: match.id,
        });
        return match.id;
      ...
      if (!err.status)
        logger.error("Failed resolving itemId by name", {
          driveId,
          itemName,
          error: err.message,
        });
  ...
  async recursiveSearchForFile(
    ...
    if (depth > maxDepth) {
      logger.warn(
        `Maximum search depth (${maxDepth}) reached. Stopping recursive search.`,
        { driveId, fileName, currentPath }
      );
      return [];
    }
    ...
    try {
      ...
      for (const item of items) {
        if (!item.folder && String(item.name).toLowerCase() === fileNameLc) {
          ...
          logger.debug(`Found file match: ${fullPath}`, {
            driveId,
            itemId: item.id,
          });
        }
  ...
  async resolveItemIdByPath(accessToken, driveId, itemName, itemPath) {
    ...
    try {
      ...
      if (!match) {
        ...
        const msg = `File '${itemName}' not found at path '${itemPath}'. Available paths: ${JSON.stringify(
          availablePaths
        )}`;
        logger.warn(msg, { driveId });
        throw new AppError(msg, 404);
      ...
      this.itemCache.set(cacheKey, { id: match.id, ts: Date.now() });
      logger.info(`File '${itemName}' resolved by path: ${itemPath}`, {
        driveId,
        itemId: match.id,
      });
      return match.id;
      ...
      if (!err.status)
        logger.error("Failed resolving itemId by path", {
          driveId,
          itemName,
          itemPath,
          error: err.message,
  ...
  async resolveWorksheetIdByName(accessToken, driveId, itemId, worksheetName) {
    ...
    try {
      ...
      if (!err.status)
        logger.error("Failed resolving worksheetId by name", {
          driveId,
          itemId,
          worksheetName,
          error: err.message,
