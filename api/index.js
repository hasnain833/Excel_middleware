...
const excelFunctions = {
  arithmetic: ['SUM', 'AVERAGE', 'COUNT', 'MIN', 'MAX', 'ROUND', 'ROUNDUP', 'ROUNDDOWN', 'ABS', 'POWER', 'SQRT'],
  lookup: ['VLOOKUP', 'HLOOKUP', 'INDEX', 'MATCH', 'XLOOKUP', 'FILTER', 'SORT', 'UNIQUE'],
  text: ['CONCATENATE', 'CONCAT', 'LEFT', 'RIGHT', 'MID', 'LEN', 'UPPER', 'LOWER', 'PROPER', 'TRIM'],
  logical: ['IF', 'IFS', 'AND', 'OR', 'NOT', 'TRUE', 'FALSE', 'IFERROR', 'IFNA'],
  date: ['TODAY', 'NOW', 'DATE', 'TIME', 'YEAR', 'MONTH', 'DAY', 'WEEKDAY', 'DATEDIF'],
  financial: ['PMT', 'PV', 'FV', 'RATE', 'NPER', 'NPV', 'IRR', 'XIRR'],
  statistical: ['STDEV', 'STDEVP', 'VAR', 'VARP', 'CORREL', 'MEDIAN', 'MODE', 'PERCENTILE'],
  engineering: ['CONVERT', 'BIN2DEC', 'BIN2HEX', 'DEC2BIN', 'DEC2HEX', 'HEX2BIN', 'HEX2DEC'],
...
app.get('/api/docs', (req, res) => {
  res.json({
    ...
    version: '1.0.0',
    environment: 'serverless',
    description: 'Comprehensive Excel operations via Microsoft Graph API with universal name-based resolution',
    ...
    endpoints: {
      ...
      findReplace: {
        ...
        description: 'Find and replace text with intelligent scoping and preview',
      ...
      format: {
        ...
        operationTypes: ['highlight', 'textStyle', 'borders', 'resizeColumn', 'mergeCells', 'formula', 'conditionalFormatting', 'pivotTable']
      },
      validateFormula: {
        method: 'POST',
      ...
      functions: {
        ...
        description: 'Get available Excel functions by category (no auth required)',
    ...
    authentication: {
      ...
      description: 'Most endpoints require authentication via Azure AD service principal',
    ...
    nameResolution: {
      ...
      examples: {
        legacy: { driveId: 'b!abc123', itemId: 'def456', worksheetId: 'ghi789' },
        modern: { driveName: 'Documents', fileName: 'Budget.xlsx', sheetName: 'Summary' },
        hybrid: { driveId: 'b!abc123', fileName: 'Budget.xlsx', sheetName: 'Summary' }
      },
      pathResolution: {
...
protectedEndpoints.forEach(endpoint => {
  app.all(endpoint, (req, res) => {
    res.status(401).json({
      ...
      error: {
        code: 401,
        message: 'Authentication required. Please provide a valid Azure AD access token.',
        endpoint: endpoint,
        method: req.method,
  ...
  res.status(404).json({
    ...
    error: {
      ...
      availableEndpoints: [
        ...
        'GET / (public)',
        'GET /api/docs (public)',
        'GET /api/excel/functions (public)',
        'POST /api/excel/read (auth required)',
        'POST /api/excel/write (auth required)',
        'POST /api/excel/find-replace (auth required)',
        'POST /api/excel/format (auth required)'
      ]
    },
    timestamp: new Date().toISOString()
  });
